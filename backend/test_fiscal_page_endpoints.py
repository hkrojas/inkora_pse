...
def test_guardar_respuesta_sunat_persiste_trazabilidad_smartpse(db_session):
    ...
    updated = crud.guardar_respuesta_sunat(
        ...
        doc.id,
        {
            ...
            "provider_document_name": "20123456789-01-F001-000001",
            "provider_verification_status": "verified",
            "provider_verified_at": "2026-06-23T12:00:00+00:00",
            "provider_verification_error": None,
        },
        tenant_id=tenant.id,
    )
    ...
    assert updated.provider_document_name == "20123456789-01-F001-000001"
    assert updated.provider_verification_status == "verified"
    assert updated.provider_verified_at is not None
    assert updated.provider_verification_error is None


def test_guardar_respuesta_sunat_no_factura_smartpse_sin_verificacion(db_session):
    import crud
    ...
    assert updated.sunat_error == "Smart PSE remote verification missing"
    assert updated.provider_verification_status == "failed"
    assert updated.sunat_accepted is False
