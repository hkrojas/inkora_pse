    ...
    for item in source_items:
        items_clonados.append(
            models.CotizacionItem(
                ...
                inventory_source_item_id=item.id,
...
def _smartpse_series_floor_for_tenant(db: Session, tenant_id: int, serie: str) -> int:
    ...
    configured_pairs = (
        (
            getattr(tenant, "fiscal_invoice_series", None),
            getattr(tenant, "fiscal_invoice_series_floor", None),
        ),
        (
            getattr(tenant, "fiscal_boleta_series", None),
            getattr(tenant, "fiscal_boleta_series_floor", None),
        ),
    )
