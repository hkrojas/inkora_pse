...
import { getEmissionJobLabel, getEmissionOutcome, isEmissionJobActive } from '../lib/utils/emissionJobs';
...
export default function NotaNuevaPage() {
  ...
  const pollJob = async (jobId) => {
    try {
      ...
      if (isEmissionJobActive(job.status)) {
        pollRef.current = setTimeout(() => pollJob(jobId), 1800);
      } else if (job.status === 'succeeded') {
        toast('Aceptada por SUNAT', 'success');
        navigate('/notas');
      } else if (['contingency_pending', 'pending_confirmation'].includes(job.status)) {
        const outcome = getEmissionOutcome({ job_status: job.status }, 'Nota');
  ...
  const emit = async () => {
    ...
    try {
      ...
      const outcome = getEmissionOutcome(response, 'Nota');
      setJobStatus(outcome.jobStatus);
      toast(outcome.message, outcome.toastType);
  ...
  return (
    <div className="page-shell pb-28 lg:pb-8">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        {jobStatus && <span className="badge badge-warning" role="status">{getEmissionJobLabel(jobStatus)}</span>}
      </header>
        ...
        <main className="space-y-5">
          ...
          <section className="card p-5" aria-labelledby="reason-title">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div><label className="form-label" htmlFor="note-type">Tipo de nota</label><select id="note-type" className="input w-full" value={form.tipo_nota} onChange={(event) => { setField('tipo_nota', event.target.value); setField('cod_motivo', ''); }}><option value="credito">Nota de crédito (07)</option><option value="debito">Nota de débito (08)</option></select></div>
              <div><label className="form-label" htmlFor="note-reason">Motivo</label><select id="note-reason" className="input w-full" disabled={!context} value={form.cod_motivo} onChange={(event) => setField('cod_motivo', event.target.value)}><option value="">Selecciona el motivo</option>{Object.entries(motives).map(([code, label]) => <option key={code} value={code}>{code} · {label}</option>)}</select></div>
            </div>
          ...
          {context && form.cod_motivo && <section className="card p-5" aria-labelledby="adjustment-title">
            ...
            {['global', 'payment_terms', 'charge'].includes(adjustmentMode) && <div className="mt-4 grid gap-4 md:grid-cols-2">
              {adjustmentMode === 'global' && <div><label className="form-label" htmlFor="input-type">Calcular por</label><select id="input-type" className="input w-full" value={form.input_type} onChange={(event) => setField('input_type', event.target.value)}><option value="amount">Monto</option><option value="percentage">Porcentaje</option></select></div>}
            ...
            {form.inventory_impact === 'physical_return' && <div className="mt-4"><label className="form-label" htmlFor="return-warehouse">Almacén que recibirá la devolución</label><select id="return-warehouse" className="input w-full" value={form.inventory_return_warehouse_id} onChange={(event) => setField('inventory_return_warehouse_id', event.target.value)}><option value="">Selecciona un almacén</option>{(context.warehouses || []).map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}{warehouse.is_default ? ' · Principal' : ''}</option>)}</select></div>}
          </section>}
