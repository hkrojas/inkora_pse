...
export default function InventarioPage() {
  ...
  return (
    <main className="inventory-page mx-auto w-full max-w-[1500px] space-y-5 pb-10">
      ...
      <Drawer open={modal === 'adjust'} onClose={() => setModal(null)} variant="inventory-action" eyebrow="Kardex manual" status="Entrada o salida" initialFocus="select" title="Registrar movimiento" subtitle="Ajusta existencias con un motivo que quedará visible en el kardex." icon={<Plus size={20} />} footer={<><button type="button" className="btn-ghost" onClick={() => setModal(null)}>Cancelar</button><button type="submit" form="adjust-form" className="btn-primary" disabled={saving}>{saving ? 'Guardando…' : 'Guardar movimiento'}</button></>}>
        <form id="adjust-form" onSubmit={submitAdjustment} className="inventory-action-form">
          <section className="inventory-form-section"><div className="inventory-form-section__heading"><span>01</span><div><h3>Destino del movimiento</h3><p>Selecciona dónde y sobre qué producto se aplicará.</p></div></div><label>Almacén<select required className="input mt-1" value={form.warehouse_id} onChange={(event) => setForm({ ...form, warehouse_id: event.target.value })}><option value="">Selecciona</option>{warehouses.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label><label>Producto<select required className="input mt-1" value={form.product_id} onChange={(event) => setForm({ ...form, product_id: event.target.value })}><option value="">Selecciona</option>{uniqueProducts.map((row) => <option key={row.product_id} value={row.product_id}>{row.product_name}</option>)}</select></label></section>
      ...
      <Drawer open={modal === 'config'} onClose={() => setModal(null)} variant="inventory-action" eyebrow="Catálogo comercial" status="Control de stock" initialFocus="input" title="Configurar producto" subtitle="Define cómo se comporta un producto dentro del inventario." icon={<PackageCheck size={20} />} footer={<><button type="button" className="btn-ghost" onClick={() => setModal(null)}>Cancelar</button><button type="submit" form="config-form" className="btn-primary" disabled={saving}>{saving ? 'Guardando…' : 'Guardar configuración'}</button></>}>
        <form id="config-form" onSubmit={submitConfig} className="inventory-action-form"><section className="inventory-form-section"><div className="inventory-form-section__heading"><span>01</span><div><h3>Producto del catálogo</h3><p>Busca por nombre o SKU y selecciona el resultado correcto.</p></div></div><label>Buscar producto<input className="input mt-1" value={productQuery} onChange={(event) => setProductQuery(event.target.value)} placeholder="Escribe nombre o SKU" /></label><label>Producto<select required className="input mt-1" value={config.product_id} onChange={(event) => setConfig({ ...config, product_id: event.target.value })}><option value="">Selecciona un resultado</option>{productOptions.map((product) => <option key={product.id} value={product.id}>{product.nombre}{product.codigo_interno ? ` · ${product.codigo_interno}` : ''}</option>)}</select></label></section><section className="inventory-form-section"><div className="inventory-form-section__heading"><span>02</span><div><h3>Reglas de inventario</h3><p>Los servicios no generan ni consumen existencias.</p></div></div><label>Tipo<select className="input mt-1" value={config.item_type} onChange={(event) => setConfig({ ...config, item_type: event.target.value })}><option value="inventory">Producto inventariable</option><option value="service">Servicio sin stock</option></select></label>{config.item_type === 'inventory' && <><label>Almacén<select required className="input mt-1" value={config.warehouse_id} onChange={(event) => setConfig({ ...config, warehouse_id: event.target.value })}><option value="">Selecciona</option>{warehouses.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label><div className="inventory-form-grid"><label>Saldo inicial<input type="number" step="0.0001" className="input mt-1" value={config.opening_stock} onChange={(event) => setConfig({ ...config, opening_stock: event.target.value })} /></label><label>Stock mínimo<input type="number" min="0" step="0.0001" className="input mt-1" value={config.minimum_stock} onChange={(event) => setConfig({ ...config, minimum_stock: event.target.value })} /></label></div></>}</section></form>
      </Drawer>

      <Drawer open={modal === 'transfer'} onClose={() => setModal(null)} variant="inventory-action" tone="warning" eyebrow="Movimiento interno" status="Entre almacenes" initialFocus="select" title="Nueva transferencia" subtitle="La salida y la entrada quedarán enlazadas en el kardex." icon={<ArrowLeftRight size={20} />} footer={<><button type="button" className="btn-ghost" onClick={() => setModal(null)}>Cancelar</button><button type="submit" form="transfer-form" className="btn-primary" disabled={saving}>{saving ? 'Registrando…' : 'Registrar transferencia'}</button></>}>
        <form id="transfer-form" onSubmit={submitTransfer} className="inventory-action-form"><section className="inventory-form-section"><div className="inventory-form-section__heading"><span>01</span><div><h3>Ruta del stock</h3><p>El almacén de destino debe ser diferente al de origen.</p></div></div><div className="inventory-form-grid"><label>Origen<select required className="input mt-1" value={transfer.source_warehouse_id} onChange={(event) => setTransfer({ ...transfer, source_warehouse_id: event.target.value })}><option value="">Selecciona</option>{warehouses.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label><label>Destino<select required className="input mt-1" value={transfer.destination_warehouse_id} onChange={(event) => setTransfer({ ...transfer, destination_warehouse_id: event.target.value })}><option value="">Selecciona</option>{warehouses.filter((row) => String(row.id) !== String(transfer.source_warehouse_id)).map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label></div></section><section className="inventory-form-section"><div className="inventory-form-section__heading"><span>02</span><div><h3>Detalle del traslado</h3><p>Indica qué producto y cuántas unidades se moverán.</p></div></div><label>Producto<select required className="input mt-1" value={transfer.product_id} onChange={(event) => setTransfer({ ...transfer, product_id: event.target.value })}><option value="">Selecciona</option>{uniqueProducts.map((row) => <option key={row.product_id} value={row.product_id}>{row.product_name}</option>)}</select></label><label>Cantidad<input required type="number" min="0.0001" step="0.0001" className="input mt-1" value={transfer.quantity} onChange={(event) => setTransfer({ ...transfer, quantity: event.target.value })} /></label><label>Motivo<textarea required minLength={3} className="input mt-1 min-h-24" value={transfer.reason} onChange={(event) => setTransfer({ ...transfer, reason: event.target.value })} placeholder="Ej. Reposición de tienda" /></label></section></form>
      </Drawer>
